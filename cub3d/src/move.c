...
/*   Updated: 2020/10/20 14:26:45 by tkleynts         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				move_fw(t_cub *data)
{
	...
	if (data->map[(int)data->pos.y]
				[(int)(new_pos.x - data->move_ud * space.x)] == '0')
	{
		setpos(data, new_pos.x, data->pos.y);
		ret = 1;
	}
	if (data->map[(int)(new_pos.y + data->move_ud * space.y)]
				[(int)data->pos.x] == '0')
	{
