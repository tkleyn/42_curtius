...
/*   Updated: 2020/01/17 14:36:50 by tkleynts         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char			*conv_s(t_utils *data, t_flags *flgs, char *str)
{
	...
	if (flgs->pad && flgs->width && ft_add_l(&str, flgs->width, '0') != 0)
		return (NULL);
	if (flgs->width >= 0 && ft_add_l(&str, flgs->width, ' ') != 0)
		return (NULL);
...
char			*conv_diuxp(t_utils *data, t_flags *flgs, char *str)
{
    ...
    if (flgs->pad && !flgs->ljust && flgs->width > 0 && flgs->prec == -1 
						&& ft_add_l(&str, flgs->width, '0') != 0)
    	return (NULL);
	if (flgs->pad && !flgs->ljust && flgs->width > 0 && ft_add_l(&str, flgs->width, ' ') != 0)
    	return (NULL);
	return (str);
}
